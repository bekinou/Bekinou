# 👋 Hi there, I'm Bekinou

## 🛡️ Cybersecurity & Network Engineering Student (Graduating 2026)

Passionate about offensive security, infrastructure hardening and secure cloud architecture.  
3 years of experience in IT Support & Cybersecurity with a strong focus on Active Directory security, penetration testing and cloud infrastructure design.

---

## 🔧 What I'm working on

- 🎯 Advanced penetration testing (Active Directory, Web Applications)
- ☁️ Secure cloud architecture design & infrastructure automation
- 🔍 Security auditing aligned with NIST CSF & CIS Controls
- 🚀 Building technical troubleshooting labs for cybersecurity training

---

## 🚀 Personal Initiative – UCSN (Unified Cloud Security & Network)

Founder of UCSN, a long-term infrastructure project focused on designing secure, optimized and scalable cloud environments.

UCSN explores:
- Security-by-design architectures  
- Infrastructure cost optimization (FinOps mindset)  
- Infrastructure as Code & automated deployments  
- Secure access management & network segmentation  
- Monitoring and observability strategies  

Currently in active development as a technical and strategic lab.

---

## 🏆 Certifications

- ✅ eCPPT – Certified Professional Penetration Tester  
- ✅ eJPT – Junior Penetration Tester  
- 🔄 AZ-500 – Azure Security Engineer Associate (in progress)  
- 🔄 CEH – Certified Ethical Hacker (in progress)  
- ✅ CCNA – Cisco Certified Network Associate  

---

## 🛠️ Tech Stack

**Security:** Metasploit | Burp Suite | Wireshark | Nmap | Impacket | Evil-WinRM  
**Cloud & DevSecOps:** AWS | Terraform | Docker | Ansible  
**Languages:** Python | Bash | PowerShell  

---

💡 *Security is not a feature, it's a foundation.*

## 📫 Connect with me:

- 💼 LinkedIn: [Your LinkedIn][(https://linkedin.com/in/yourprofile)](https://www.linkedin.com/in/bereket-tsigie-752187222/)
- 📧 Email: berekettkelesi@gmail.com

---

💡 *"Always learning, always hacking"*
